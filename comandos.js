...
END:VCARD`.trim();
  ...
  try {
    ...
    for (const jid of alvos) {
      ...
      if (jid === botNumber) {
        await sock.sendMessage(from, {
          text: '🤖 Não posso me auto promover.'
        }, { quoted: msg });
        continue;
      }
      try {
  ...
  const link = "";
  const titulo = "✨𝐆𝐑𝐔𝐏𝐎 𝑺𝑶𝑴𝑩𝑹𝑨 𝟐𝟗𝟏 • 𝑪𝑯𝑨𝑻";
...
// 🔹 A partir daqui começam os comandos
  ...
  const status = isBotoff ? '🔕 Bot desligado neste grupo.' : '🔔 Bot ligado neste grupo.';
  ...
  await sock.sendMessage(from, {
    ...
    contextInfo: {
      ...
      externalAdReply: {
        ...
        mediaType: 1,
        showAdAttribution: false,
        sourceUrl: 'https://chat.whatsapp.com/B6OaB3K4A5G2BJnjiXD3t3?mode=ems_copy_t'
      }
    }
  }, { quoted: msg });
}
